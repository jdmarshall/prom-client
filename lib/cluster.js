...
class AggregatorRegistry extends Registry {
	...
	async clusterMetrics() {
		...
		const promises = [this.#selfMetrics(), ...workerMetrics];
		const request = {
			responseHandlers,
			promise: waitFor(this.#gather(requestId, promises), 5_000),
		};
	 ...
	 */
	async #gather(requestId, promises) {
		const responses = await Promise.all(promises);
		const metrics = responses.flatMap(response => response.metrics);
		return Registry.aggregate(metrics).metrics();
	}
