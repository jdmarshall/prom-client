...
describe('T-Digests in which each point becomes a centroid', () => {
	it('consumes a point', () => {
		...
		expect(points).toStrictEqual([{ mean: 0, n: 1 }]);
	});
	...
	it('consumes increasing-valued points', () => {
		...
		const points = tdigest.toArray();
		for (i = 0; i < N; i += 1) {
			expect(points[i].mean).toEqual(i * 10);
		}
	});
...
describe('T-Digests in which points are merged into centroids', () => {
	it('consumes same-valued points into a single point', () => {
		...
		const points = tdigest.toArray();
		expect(points).toStrictEqual([{ mean: 1000, n: N }]);
	});
	...
	it('handles multiple duplicates', () => {
		...
		expect(tdigest.toArray()).toStrictEqual([
			{ mean: 0.0, n: N },
			{ mean: 0.5, n: N },
			{ mean: 1.0, n: N },
		]);
...
describe('compress', () => {
	it('compresses points and preserves bounds', () => {
		...
		expect(tdigest.size()).toEqual(100);
		tdigest.delta = 0.1; // encourage merging (don't do this!)
		tdigest.compress();
		const points = tdigest.toArray();
		expect(points.length).toBeLessThan(100);
		expect(points[0].mean).toEqual(0);
		expect(points.at(-1).mean).toEqual((N - 1) * 10);
	});
	...
	it('K automatically compresses during ingest', () => {
		...
		const points = tdigest.toArray();
		expect(tdigest.nreset).toBeGreaterThan(1);
		expect(points.length).toBeLessThan(10000);
		expect(points[0].mean).toEqual(0);
		expect(points.at(-1).mean).toEqual(99990);
	});
});
...
describe('percentile ranks', () => {
	...
	it('reports undefined when given no points', () => {
		const tdigest = new TDigest();
		const x = [1, 2, 3];
		expect(tdigest.p_rank(1)).toBeUndefined();
		expect(tdigest.p_rank(x)).toStrictEqual([undefined, undefined, undefined]);
	});
	...
	it('from a single point', () => {
		...
		const q = [0, 0.5, 1, 1, 1];
		expect(tdigest.p_rank(x)).toEqual(q);
	});
	...
	it('from two points', () => {
		...
		expect(tdigest.p_rank(x)).toStrictEqual(q);
	});
	...
	it('from an exact match', () => {
		...
		expect(tdigest.p_rank(20)).toEqual(0.5);
	});
});
...
describe('percentiles', () => {
	it('reports undefined when given no points', () => {
		const tdigest = new TDigest();
		const p = [0, 0.5, 1.0];
		expect(tdigest.percentile(0.5)).toBeUndefined();
		expect(tdigest.percentile(p)).toStrictEqual([
			undefined,
			undefined,
			undefined,
		]);
	...
	it('from a single point', () => {
		...
		const p = [0, 0.5, 1.0];
		const x = [0, 0, 0];
		expect(tdigest.percentile(p)).toStrictEqual(x);
	});
